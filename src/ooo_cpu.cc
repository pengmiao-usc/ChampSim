...
void O3_CPU::handle_branch()
{
    ...
    while (continue_reading) {
        if (!fread(&current_instr, sizeof(input_instr), 1, trace_file)) {
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
    ...
    if (is_it_tlb) {
        ...
        ROB.entry[rob_index].instruction_pa = (queue->entry[index].instruction_pa << PAGE_SHIFT) | (ROB.entry[rob_index].ip & ((1 << PAGE_SHIFT) - 1)); // translated address
    }
    else
        ...
        for (uint32_t i=0; i<ROB_SIZE; i++) {
            if (queue->entry[index].rob_index_depend_on_me[i]) {
                ...
                if (is_it_tlb) {
                    ...
                    ROB.entry[i].instruction_pa = (queue->entry[index].instruction_pa << PAGE_SHIFT) | (ROB.entry[i].ip & ((1 << PAGE_SHIFT) - 1)); // translated address
                }
                else
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
    ...
    if (is_it_tlb) { // DTLB
        ...
        if (queue->entry[index].type == RFO) {
            SQ.entry[sq_index].physical_address = (queue->entry[index].data_pa << PAGE_SHIFT) | (SQ.entry[sq_index].virtual_address & ((1 << PAGE_SHIFT) - 1)); // translated address
            ...
            LQ.entry[lq_index].physical_address = (queue->entry[index].data_pa << PAGE_SHIFT) | (LQ.entry[lq_index].virtual_address & ((1 << PAGE_SHIFT) - 1)); // translated address
            LQ.entry[lq_index].translated = COMPLETED;
            LQ.entry[lq_index].event_cycle = current_core_cycle[cpu];
...
void O3_CPU::handle_o3_fetch(PACKET *current_packet, uint32_t cache_type)
{
    ...
    if (cache_type == 0) { // DTLB
        ...
        if (current_packet->type == RFO) {
            SQ.entry[sq_index].physical_address = (current_packet->data_pa << PAGE_SHIFT) | (SQ.entry[sq_index].virtual_address & ((1 << PAGE_SHIFT) - 1)); // translated address
            SQ.entry[sq_index].translated = COMPLETED;
            ...
            LQ.entry[lq_index].physical_address = (current_packet->data_pa << PAGE_SHIFT) | (LQ.entry[lq_index].virtual_address & ((1 << PAGE_SHIFT) - 1)); // translated address
            LQ.entry[lq_index].translated = COMPLETED;
    ...
    if (provider->store_merged) {
        for (uint32_t merged=0; merged<SQ.SIZE; merged++) {
            if (provider->sq_index_depend_on_me[merged]) {
                ...
                SQ.entry[merged].physical_address = (provider->data_pa << PAGE_SHIFT) | (SQ.entry[merged].virtual_address & ((1 << PAGE_SHIFT) - 1)); // translated address
    ...
    if (provider->load_merged) {
        for (uint32_t merged=0; merged<LQ.SIZE; merged++) {
            if (provider->lq_index_depend_on_me[merged]) {
                ...
                LQ.entry[merged].physical_address = (provider->data_pa << PAGE_SHIFT) | (LQ.entry[merged].virtual_address & ((1 << PAGE_SHIFT) - 1)); // translated address
